...
bool flag_eeprom_write_mode = false; // EEPROMへの書き込みモード.
bool flag_eeprom_read_mode = false; // EEPROMへの書き込みモード.
bool flag_sdcard_write_mode = false; // SDCARDへの書き込みモード.
bool flag_sdcard_read_mode = false;  // SDCARDへの書き込みモード.
...
void loop()
{
    ...
    // EEPROMやSDへの読み書き処理はおそらくここで実施.
    // execute_MasterCommand()関数内でフラグを立て, フラグによって分岐.
    // その際, @[6-1]から @[10-4] のプロセスを適宜キャンセル.
    //

    //------------------------------------------------------------------------------------
    //---- < 6 > センサー類読み取り ---------------------------------------------------------
    //------------------------------------------------------------------------------------
    // @[6-1] IMU/AHRSについてはタイマー割り込みで別途処理
    ...
    //---- < 9 > サーボコマンドの書き込み ----------------------------------------------------
    //------------------------------------------------------------------------------------
    // @[9-1] Meridim配列をサーボ命令に変更

    // @[9-2] サーボコマンドの配列に書き込み

    // @[9-3] サーボデータのICS送信および返り値を取得


    //------------------------------------------------------------------------------------
    //---- < 10 > サーボ動作の実行 -----------------------------------------------------------
    //------------------------------------------------------------------------------------
    // @ [10-1] サーボ命令の実行およびサーボ角度戻り値の取得
    ...
    // @[11-4] サーボIDごとにの現在位置もしくは計算結果を配列に格納
    for (int i = 0; i < 15; i++)
    {
    ...
    // @[11-5] Meridimのシーケンス番号をカウントアップして送信用に格納
    ...
    //---- < 12 > フレーム終端処理 ----------------------------------------------------------
    //------------------------------------------------------------------------------------
    // @[12-1] 必要に応じてフレーム管理時計mrd_t_milを現在時刻にリセット
    ...
    // @[12-2] この時点で１フレーム内に処理が収まっていない時の処理
    if (!flag_udp_board_passive) // パッシブモードの時は末端処理を行わない
    {
        ...
        // @[12-3] この時点で時間が余っていたら時間消化. 時間がオーバーしていたらこの処理を自然と飛ばす.
        now_t_mil = (long)millis();
    ...
    {
        delay(5); // 負荷低減のため5msのディレイ
    }

    // @[12-4]フレーム管理時計mercのカウントアップ
...
void execute_MasterCommand_1()
{
    ...
    {
        flag_eeprom_write_mode = true; // 書き込みモードのフラグを上げる
